...
const CommunityMembersTab: React.FC<Props> = ({ moduleId, moduleName, color, isAdmin = false }) => {
  ...
  // Regular members should only see members who have officially joined / approved
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [visibleEnrollments, search, statusFilter, isAdmin]);
  ...
  const getYearInfo = (m: any) => {
    if (m.year_of_study || m.academic_year || m.year) {
      ...
      return typeof val === 'number' || !val.toLowerCase().includes('year') ? `Year ${val}` : val;
    }
    ...
    <div className="tab-system-content" style={{ '--jumuiya-color': color } as React.CSSProperties}>
      ...
      <div className="tab-header-wrap">
        <div className="header-text">
          <h1 className="page-title">{isAdmin ? 'Registered Members' : 'Community Members'}</h1>
          <p className="page-description">
            {isAdmin
              ? `${allEnrollments.length} registered member${allEnrollments.length !== 1 ? 's' : ''} in ${moduleName}`
              : `${visibleEnrollments.length} active member${visibleEnrollments.length !== 1 ? 's' : ''} in ${moduleName}`}
          </p>
        </div>
      </div>

      {/* Admin-only stats breakdown */}
      {isAdmin && (
      ...
      {isLoading ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3.5 mt-1">
                    <div
                      className="w-13 h-13 rounded-2xl overflow-hidden shrink-0 shadow-md ring-2 transition-transform group-hover:scale-105"
                      style={{ ringColor: `${color}20`, width: '52px', height: '52px' }}
                    >
                      {member.profile_image ? (
                        <img src={member.profile_image} alt={name} className="w-full h-full object-cover" />
                      ) : (
                        <div
                          className="w-full h-full flex items-center justify-center font-black text-base text-white"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Year badge for regular members & admins */}
                  ...
                  {/* Admin-only contact action links */}
                  {isAdmin && (
        ...
        <div className="text-center py-16 rounded-3xl" style={{ background: `${color}06`, border: `1px dashed ${color}25` }}>
          ...
          <p className="font-semibold text-slate-400 text-sm">
            {search ? 'No members match your search.' : 'No members registered yet. Be the first to join!'}
          </p>
        </div>
      )}
    </div>
