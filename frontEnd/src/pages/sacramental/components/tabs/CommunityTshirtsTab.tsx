...
const TshirtProductCard: React.FC<ProductCardProps> = ({ product, color, moduleId, variant, onShowSizeChart }) => {
  ...
  return (
    <div
      className={`rounded-3xl p-6 overflow-hidden relative ${isWide ? 'max-w-2xl mx-auto' : ''}`}
      style={{
    ...
    >
      ...
      <div className={`relative z-10 flex flex-col ${isWide ? 'md:flex-row' : ''} gap-6`}>
        {/* Image */}
        <div className={isWide ? 'md:w-1/2 shrink-0' : ''}>
          <div className="rounded-2xl overflow-hidden border border-slate-100 bg-slate-50 shadow-sm" style={{ aspectRatio: isWide ? '1 / 1' : '4 / 3' }}>
            <img
        ...
        {/* Details */}
        <div className={`flex flex-col ${isWide ? 'md:w-1/2' : ''}`}>
          <div className="mb-4">
            {product?.name && (
        ...
        products.length === 0 ? (
          <div className="text-center py-16 rounded-3xl" style={{ background: `${color}06`, border: `1px dashed ${color}25` }}>
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-4" style={{ background: `${color}10` }}>
              <FaTshirt style={{ color: `${color}40` }} size={28} />
            </div>
            <p className="font-semibold text-slate-400 text-sm">No t-shirts available yet.</p>
          </div>
        ) : products.length === 1 ? (
          <TshirtProductCard
