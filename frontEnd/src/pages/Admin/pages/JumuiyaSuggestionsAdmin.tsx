...
export default function JumuiyaSuggestionsAdmin() {
  ...
  const handleRequestUnmask = async (id: number) => {
    if (
      !window.confirm(
        'Request to unmask this anonymous suggestion? Designated co-approvers (Chairperson & Secretary) must approve.'
      )
    )
      return;
    setUnmaskLoading(id);
    try {
      ...
      toast.success(res.data?.message || 'Unmask request sent to Chairperson and Secretary');
      loadSuggestions();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      {/* Header Banner - Matching CSA Vice Chairperson Style */}
      ...
      {loading && suggestions.length === 0 ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {filteredSuggestions.map((item) => (
            <div
              key={item.id}
              className="bg-white hover:bg-slate-50 transition-all duration-300 rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col group"
            >
              <div className="p-6 md:p-8 flex-1">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    {item.status && (
                      <span
                        className={`px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider ${
                          STATUS_COLORS[item.status] || 'bg-slate-50 text-slate-500 border-slate-200'
                        }`}
                      >
                    ...
                    {item.category && (
                      <span
                        className={`px-2.5 py-1 rounded-full text-[10px] font-bold border tracking-wider ${
                          CATEGORY_COLORS[item.category] || 'bg-slate-50 text-slate-500 border-slate-200'
                        }`}
                      >
                        {item.category}
                      </span>
                  ...
                  {item.name ? (
                    ...
                    <div className="flex items-center gap-3 text-slate-600">
                      ...
                      <span className="text-xs font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">
                        Year {item.member_year_of_study}
                      </span>
                      <span className="text-xs text-slate-300">|</span>
                    ...
                    <div className="flex items-center gap-2 text-slate-400">
                      <User size={14} />
                      <span className="text-sm font-bold">Anonymous Member</span>
                    </div>
                  )}
