...
export default function GalleryManager({ jumuiyaId, jumuiyaInfo }: Props = {}) {
  ...
  // Video state (only for global gallery, not jumuiya)
  ...
  useEffect(() => {
    if (galleryView === 'videos' && !jumuiyaId) {
      loadVideos();
    }
  }, [galleryView, jumuiyaId]);
  ...
  const loadVideos = async () => {
    ...
    try {
      const { data } = await apiClient.get(`/community-videos/${csaMainModuleId}/videos`);
  ...
  const handleAddLink = async () => {
    if (!linkForm.video_url || !linkForm.platform) return;
    setLinkSaving(true);
    try {
      await apiClient.post(`/community-videos/${csaMainModuleId}/videos`, {
  ...
  const handleDeleteVideo = async (videoId: number) => {
    ...
    try {
      await apiClient.delete(`/community-videos/${csaMainModuleId}/videos/${videoId}`);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-700 text-xs mt-0.5">
            {jumuiyaInfo ? `Manage photos for ${jumuiyaInfo.name}` : 'Manage public photos and videos for the church website.'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {!jumuiyaId && (
            <div className="flex p-1 bg-slate-100 rounded-xl">
              {(['photos', 'videos'] as const).map(view => (
                ...
                >
                  {view === 'photos' ? <ImageIcon size={14} /> : <Video size={14} />}
          ...
          <div className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-[10px] font-bold border border-blue-100">
            {galleryView === 'photos' ? `${images.length} Photos` : `${videos.length}/${MAX_GALLERY_VIDEOS} Videos`}
          </div>
          {!jumuiyaId && (
      ...
      <div className="space-y-4">
            ...
            {videos.length < MAX_GALLERY_VIDEOS && (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-500 text-white rounded-xl text-xs font-black hover:bg-blue-600 transition shadow-sm cursor-pointer shrink-0"
                >
        ...
        {videosLoading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ml-3 text-xs font-bold text-slate-600">Loading videos...</span>
          </div>
        ) : videos.length === 0 ? (
          <div className="bg-white p-12 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center">
            <Video size={40} className="text-slate-200 mb-3" />
            <p className="text-sm font-bold text-slate-700">No videos uploaded yet</p>
            <p className="text-xs text-slate-400 mt-1">Upload videos or add links to YouTube/TikTok</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {videos.map(video => (
              <div key={video.id} className="group relative bg-slate-50 rounded-2xl overflow-hidden border border-slate-100">
                <div className="relative aspect-video bg-slate-100">
                  {video.thumbnail_url ? (
                    <img src={video.thumbnail_url} alt={video.title} className="w-full h-full object-cover" />
                  ) : video.video_file_url ? (
                    <video src={video.video_file_url} className="w-full h-full object-cover" preload="metadata" />
                  ) : (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      <p className="text-white font-bold text-xs truncate">{video.title || 'Untitled'}</p>
                      {video.description && (
                        <p className="text-slate-300 text-[10px] leading-snug line-clamp-2 mt-1">{video.description}</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {showAddLink && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <h3 className="font-black text-lg text-slate-800">Add Video Link</h3>
                <button onClick={() => setShowAddLink(false)} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full">
                  <X size={20} />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Platform</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Video URL</label>
                  <input
                    type="url"
                    value={linkForm.video_url}
                    onChange={e => setLinkForm({ ...linkForm, video_url: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:ring-2 focus:ring-blue-500 focus:outline-none"
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={linkSaving || !linkForm.video_url}
                  className="px-6 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-lg shadow-blue-200 disabled:opacity-50"
                >
                  {linkSaving ? <Loader2 size={18} className="animate-spin" /> : <Plus size={18} />}
                  {linkSaving ? 'Adding...' : 'Add Video'}
                </button>
              </div>
            </div>
          </div>
