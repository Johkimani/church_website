    ...
    <div className="space-y-5">
      {blocks.map((block, bi) => {
        if (block.type === 'heading') {
          return (
            <div key={bi} className="pt-2 pb-1 border-b border-slate-100">
              <h3 className="text-[11px] font-bold text-indigo-500 uppercase tracking-widest">{block.text}</h3>
            </div>
          );
        }
          ...
          return (
            <div key={bi} className="space-y-0.5">
              {block.items.map((item, ii) => (
                <div
                  key={ii}
                  className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-indigo-50/50 transition-colors group"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-300 group-hover:bg-indigo-500 flex-shrink-0 transition-colors" />
                  <span className="text-[13px] text-slate-800 font-medium leading-snug flex-1">
                    {item.name}
                  </span>
                  <span className="text-[13px] text-emerald-600 font-semibold italic whitespace-nowrap">
                    {item.response}
                  </span>
                </div>
              ))}
        ...
        if (block.type === 'closing') {
          return (
            <div key={bi} className="bg-slate-50 rounded-xl px-5 py-4 border border-slate-100">
              <p className="text-[13px] text-slate-700 leading-relaxed italic">{block.text}</p>
            </div>
          );
        }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      ...
      >
        ...
        <div className="border-t border-slate-100 p-4 flex justify-center">
          <button
            onClick={onClose}
            className="px-6 py-2.5 text-sm font-semibold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors"
          >
            Close
          </button>
        </div>
