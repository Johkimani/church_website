...
import type { corsOptionsType } from "./Configs/corsConfigs.js";
import type { BackendDataServiceType } from "./services/backend-data.js";
...
(BackendDataService as BackendDataServiceType).init();
...
app.use(cors(corsOptions as corsOptionsType));
...
const limiter = rateLimit({
  ...
  handler: (req: Request, res: Response, next: NextFunction) => {
    res.status(429).json({
      error: `There are too many requests. You are only allowed ${limiter.options.max
        } requests per ${limiter.options.windowMs / 60000} minutes`,
    });
  },
});
...
// Routes
app.get('/', (_req: Request, res: Response) => res.redirect('/community-hub'));
...
// Gallery APIs
app.get("/api/choir/gallery", (_req: Request, res: Response) => {
  const gallery = BackendDataService.load("choir_gallery.json", [] as any[]);
  res.json(gallery);
});
app.post("/api/choir/gallery", upload.single("file"), (req: Request, res: Response) => {
  if (! (req as any).file) return res.status(400).json({ error: "No file uploaded" });
  const gallery = BackendDataService.load("choir_gallery.json", [] as any[]);
  const newPhoto = {
    id: Date.now().toString(),
    filename: (req as any).file.filename,
    eventName: (req as any).body.eventName || "Untitled Event",
    description: (req as any).body.description || "",
    uploadDate: new Date().toISOString(),
    imageUrl: `/images/gallery/${(req as any).file.filename}`,
  };
