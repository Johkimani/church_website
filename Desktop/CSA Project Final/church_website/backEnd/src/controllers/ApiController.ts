...
export const getTableData = async (tableName: string): Promise<any[]> => {
  try {
    const result: QueryResult = await testDb.query(`SELECT * FROM ${tableName} ORDER BY id DESC`);
    return result.rows;
  } catch (error: any) {
...
export const createRecord = async (tableName: string, data: Record<string, any>): Promise<any> => {
  try {
    ...
    const result: QueryResult = await testDb.query(query, values);
...
export const deleteRecord = async (tableName: string, id: string | number): Promise<any | null> => {
  try {
    const query = `DELETE FROM ${tableName} WHERE id = $1 RETURNING *`;
    const result: QueryResult = await testDb.query(query, [id]);
    return result.rows[0];
  } catch (error: any) {
